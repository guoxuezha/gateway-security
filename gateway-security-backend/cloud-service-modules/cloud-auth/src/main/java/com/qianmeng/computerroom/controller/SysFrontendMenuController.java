...
@Slf4j
public class SysFrontendMenuController extends ApiController {

    /**
     * 服务对象
     */
    @Resource
    private SysFrontendMenuService sysFrontendMenuService;
    ...
    public Mono<ResponseResult> selectAll() {
        return Mono.just(ResponseResult.success(sysFrontendMenuService.selectAllMenu()));
    }
    ...
    public Mono<ResponseResult> insert(SysFrontendMenu sysFrontendMenu) {
        ...
        SysFrontendVo menuVo = null;
        try {
            menuVo = sysFrontendMenuService.insert(sysFrontendMenu);
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * 删除数据
     *
     * @param sysFrontendMenu 主键结合
     * @return 删除结果
     */
    @PutMapping("/delete")
    @ApiOperation(value = "修改菜单项", notes = "修改菜单项")
    @ApiImplicitParam(name = "idList", value = "要删除的菜单项ID集合", required = true, dataType = "List<String>", paramType = "path")
    public Mono<ResponseResult> delete(SysFrontendMenu sysFrontendMenu) {
        return Mono.just(ResponseResult.success(this.sysFrontendMenuService.updateByMenuId(sysFrontendMenu)));
    }
